...
(defn move-subtree
  ...
  [root left-node]
  (let [parent-node (-get-parent left-node)
        right-node (-get-right left-node)
        new-root (-> (-set-parent-id root (-get-id parent-node))
                     (-set-left-id (-get-id left-node)))
        new-right-node (-set-left-id right-node (-get-id root))]
    (-save new-root)
    (-save new-right-node)))
